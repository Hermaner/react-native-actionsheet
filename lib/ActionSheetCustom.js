...
class ActionSheet extends Component {
	...
	_renderCancelButton() {
		let {options, labelName, cancelButtonIndex, tintColor} = this.props
		if (cancelButtonIndex > -1 && options[cancelButtonIndex][labelName]) {
			return (
				<TouchableHighlight
					activeOpacity={1}
				...
				>
					<Text style={[btnStyle.title, {fontWeight: '700', color: tintColor}]}>{options[cancelButtonIndex][labelName]}</Text>
				</TouchableHighlight>
			)
		} else {
			return null
	...
	_renderOptions() {
		let {options, labelName, tintColor, cancelButtonIndex, destructiveButtonIndex} = this.props
		return options.map((title, index) => {
			let fontColor = destructiveButtonIndex === index ? WARN_COLOR : tintColor
			return index === cancelButtonIndex ? null : this._createButton(title[labelName], fontColor, index)
		})
	}
